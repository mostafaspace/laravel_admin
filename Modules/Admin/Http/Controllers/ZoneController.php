...
class ZoneController extends AdminController {
    ...
    public function getZoneData()
    {
        ...
        $zone = ZoneTranslation::join('zone', 'zone.id', '=', 'zone_translations.zone_id')
            ->select([
                'zone_translations.name as name',
                'zone_translations.locale as locale',
                'zone.id as zone_id',
            ])
            ->where(function($query) use($lang)
            {
                if($lang == 'all') {
                    $query->whereIn('zone_translations.locale', $this->language_available);
                } else {
                    $query->where('zone_translations.locale', $lang);
                }

            })
            ->orderby('zone.id', 'DESC');
