...
//        $cities = CityTranslation::all();
        $cities = CityTranslation::join('city', 'city.id', '=', 'city_translations.city_id')
        ->select([
            'city_translations.name as name',
            'city_translations.locale as locale',
            'city.id as city_id',
        ])
        ->where(function($query) use($lang)
        {
            if($lang == 'all') {
                $query->whereIn('city_translations.locale', $this->language_available);
            } else {
                $query->where('city_translations.locale', $lang);
            }
        ...
        $datatables ->addColumn('action', function ($city) {
            ...
            return '<a class="btn btn-danger btn-sm" href="'. route('admin.city.delete.post' , $city->city_id) .'"><i class="fa fa-trash"></i>delete</a>';
        });
